...
@com.qualcomm.robotcore.eventloop.opmode.TeleOp(name = "DifSwerveTele", group = "Opmodes")

public class DifTeleop extends UpliftTele
{
...
//resets encoders
            ...
            robot.getFrontLeft().setDirection(DcMotorSimple.Direction.REVERSE);
//        rt.setDirection(DcMotorSimple.Direction.REVERSE);
            robot.getBackLeft().setDirection(DcMotorSimple.Direction.REVERSE);
...
// Estimates front and back position of the wheel
            double wheelPosRightf = (((robot.getFrontRight().getCurrentPosition() + -robot.getBackRight().getCurrentPosition()) / 2) / 2.641111 ) % 360; // get wheels angle, assuming starting from forward pos
            double wheelPosLeftf =  -((((robot.getFrontLeft().getCurrentPosition() + -robot.getBackLeft().getCurrentPosition()) / 2) / 2.641111 ) % 360);
...
////                {
//                    robot.getFrontLeft().setPower((wheelPosLeftf-180)/720);
//                    robot.getBackLeft().setPower(-(wheelPosLeftf-180)/720);
////                }
//
...
//        }
            ...
            robot.getFrontLeft().setPower(((magnitude - brake) * slowMode) * magnitudeDir -  (wheelTurnPowerLeft) + turn);
            robot.getBackLeft().setPower(((magnitude - brake) * slowMode) * magnitudeDir + (wheelTurnPowerLeft) + turn);
            robot.getFrontRight().setPower(((magnitude - brake) * slowMode * magnitudeDir) -  (wheelTurnPowerRight) - turn);
            robot.getBackRight().setPower(((magnitude - brake) * slowMode * magnitudeDir) +  (wheelTurnPowerRight) - turn);
